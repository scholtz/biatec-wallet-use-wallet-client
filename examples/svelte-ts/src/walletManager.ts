...
// Get a project id at https://cloud.reown.com and put it in examples/svelte-ts/.env
// as VITE_WC_PROJECT_ID (copy .env.example).
const projectId = import.meta.env.VITE_WC_PROJECT_ID as string | undefined
if (!projectId) {
  throw new Error('Set VITE_WC_PROJECT_ID in examples/svelte-ts/.env (see .env.example)')
}
...
/**
 * The single WalletManager instance for the app. Create it once, then call
 * `useWalletContext(walletManager)` from the root component's script (App.svelte) so
 * `useWallet()`/`useNetwork()` can read it via Svelte context in any descendant.
 *
