...
/** Drive the adapter through a full connect so `session` is populated. */
...
describe('BiatecWalletAdapter', () => {
  ...
  describe('connect', () => {
    ...
    it('opens and closes the WalletConnect modal when no onDisplayUri is given', async () => {
      const session = makeSession()
      mocks.signClient.connect.mockResolvedValue({
        uri: 'wc:uri',
        approval: async () => session
      })
      const { adapter } = createAdapter({ themeMode: 'dark' })
    ...
    it('throws when the client returns no URI', async () => {
      ...
      await expect(adapter.connect()).rejects.toThrow('No URI found')
    })
    ...
    it('throws when the session has no accounts', async () => {
      ...
      const { adapter } = createAdapter({ onDisplayUri: () => undefined })
      await expect(adapter.connect()).rejects.toThrow('No accounts found!')
    })
    ...
    it('restores the last WalletConnect session and syncs accounts', async () => {
      ...
      const { adapter } = createAdapter(
        {},
        {
          wallets: {
            [WALLET_ID]: {
              accounts: [{ name: 'stale', address: STRANGER }],
              activeAccount: { name: 'stale', address: STRANGER }
            }
          }
        }
      )
